...
const grade9 = context.window.WORD_SNAP_STAGE_LISTS["初三考试词汇"].words;
assert(grade9.some((word) => word.en === "dress" && word.zh === "连衣裙"), "dress should be spelled with two s characters");
assert(grade9.some((word) => word.en === "discipline" && word.zh === "纪律"), "discipline spelling should be repaired");
assert(grade9.some((word) => word.en === "trousers" && word.zh === "裤子"), "trousers should use the normal plural clothing form");

const grade10InClass = context.window.WORD_SNAP_STAGE_LISTS["高一课内词汇"].words;
for (const [en, zh] of [["proposal", "提议,建议,动议"], ["sight", "视野,视力,看见"], ["seek", "试图,寻找,争取,寻求"], ["pressure", "心理压力,紧张,压力,要求,催促"]]) {
  assert(grade10InClass.some((word) => word.en === en && word.zh === zh), `${en} should be restored as a standalone vocabulary row`);
}

const grade10Exam = context.window.WORD_SNAP_STAGE_LISTS["高一考试词汇"].words;
assert(grade10Exam.some((word) => word.en === "shape" && word.zh === "塑造"), "shape, not sharp, should mean 塑造");

const grade12 = context.window.WORD_SNAP_STAGE_LISTS["高三考试词汇"].words;
assert(grade12.some((word) => word.en === "use" && word.zh === "使用"), "use, not users, should mean 使用");
assert(grade12.some((word) => word.en === "win" && word.zh === "获胜，赢得"), "win, not winner, should mean 获胜，赢得");
...
assert(checked > 5000, "all unique built-in vocabulary rows should be audited");
